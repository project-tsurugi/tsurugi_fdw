...
PREPARE select_t2_group1_ng
    AS SELECT count(c1) AS "count(c1)", sum(c2) AS "sum(c2)", c7 FROM t2_prepare_select_statement GROUP BY c7 ORDER BY c7;
-- GROUP BY #2
...
PREPARE select_pg_join_tg7
    AS SELECT a.*, b.* FROM t1_prepare_select_statement AS a CROSS JOIN t2_prepare_select_statement AS b ORDER BY b.c1, a.c1;
