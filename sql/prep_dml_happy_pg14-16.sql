/* Test case: happy path - Supported DML statement patterns (preparation) - PostgreSQL 14-16 */
...
-- GROUP BY ALL - Only syntax is supported
PREPARE fdw_prepare_sel AS
  SELECT ref_id, COUNT(*) AS member_num, SUM(value) AS total
    FROM fdw_select_variation_table
    GROUP BY ALL ref_id
    ORDER BY ref_id;
EXECUTE fdw_prepare_sel;
DEALLOCATE fdw_prepare_sel;

-- GROUP BY DISTINCT - Only syntax is supported
PREPARE fdw_prepare_sel AS
  SELECT ref_id, COUNT(*) AS member_num, SUM(value) AS total
    FROM fdw_select_variation_table
