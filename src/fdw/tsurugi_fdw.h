 ...
 */
typedef struct tsurugi_fdw_foreign_scan_state_
{
	TGconn	 *tg_conn;	 /* Tsurugi transaction handle */
	...
	Oid *column_types; /* Pointer to the data type (Oid) of the column to be
						  SELECT */
 ...
 */
typedef struct tsurugi_fdw_direct_modify_state_
{
 ...
 */
typedef struct tsurugi_fdw_foreign_modify_state_
{
	Relation	   rel;		  /* relcache entry for the foreign table */
	AttInMetadata *attinmeta; /* attribute datatype conversion metadata */
...
extern void deparseInsertSql(
		...
		int			  *values_end_len);
extern void deparseUpdateSql(
		StringInfo	   buf,
		RangeTblEntry *rte,
		Index		   rtindex,
		Relation	   rel,
		List		  *targetAttrs);
extern void deparseDirectUpdateSql(
		StringInfo	 buf,
		PlannerInfo *root,
		Index		 rtindex,
		Relation	 rel,
...
extern void deparseDeleteSql(
		...
		List		  *returningList,
		List		 **retrieved_attrs);
extern void deparseDirectDeleteSql(
		StringInfo	 buf,
		PlannerInfo *root,
		Index		 rtindex,
		Relation	 rel,
...
extern Expr *find_em_expr_for_input_target(
		PlannerInfo *root, EquivalenceClass *ec, PathTarget *target);
extern List *build_tlist_to_deparse(RelOptInfo *foreignrel);
extern void	 deparseSelectStmtForRel(
		 StringInfo	  buf,
		 PlannerInfo *root,
		 RelOptInfo	 *foreignrel,
		 List		 *tlist,
 ...
 * Helper functions.
 */
extern void estimate_path_cost_size(
		PlannerInfo		   *root,
		RelOptInfo		   *foreignrel,
		...
		Cost			   *p_startup_cost,
		Cost			   *p_total_cost);
extern bool ec_member_matches_foreign(
		PlannerInfo		  *root,
		RelOptInfo		  *rel,
		EquivalenceClass  *ec,
...
extern void add_paths_with_pathkeys_for_rel(
		PlannerInfo *root, RelOptInfo *rel, Path *epq_path);
extern void add_foreign_grouping_paths(
		PlannerInfo		   *root,
		RelOptInfo		   *input_rel,
		RelOptInfo		   *grouped_rel,
		GroupPathExtraData *extra);
extern void add_foreign_ordered_paths(
		PlannerInfo *root, RelOptInfo *input_rel, RelOptInfo *ordered_rel);
extern void add_foreign_final_paths(
		PlannerInfo		   *root,
		RelOptInfo		   *input_rel,
...
extern bool tsurugi_foreign_join_ok(
		PlannerInfo		  *root,
		RelOptInfo		  *joinrel,
		JoinType		   jointype,
		RelOptInfo		  *outerrel,
