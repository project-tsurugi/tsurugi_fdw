 ...
 */
const char* tg_global_error_message(void) noexcept {
...
std::string get_shared_memory_name() noexcept {
    ...
    try {
        ...
        elog(DEBUG1, 
            "tsurugi_fdw: filesystem exception occurred (what: %s)", 
            e.what());
    } catch (...) {
...
extract_prepare_statement(const char* query) noexcept {
    elog(DEBUG3, "tsurugi_fdw: %s", __func__);
...
void make_tuple_from_result_row(ResultSetPtr result_set, TupleDesc tupleDescriptor, 
        List* retrieved_attrs, Datum* row, bool* is_null) noexcept {
 ...
 *  tg_conn_connect
 */
TGconn* tg_conn_open(const char* endpoint, 
        const char *user, const char *password) noexcept {
...
TG_STATUS tg_conn_close(TGconn* tg_conn) noexcept {
    ...
    if (!tg_conn) {
        return set_error(tg_conn->error, 
                "tg_conn_close: null", TG_STATUS_INVALID_ARG);
    };
    try {
        tg_conn->tx = nullptr;
...
TG_STATUS tg_conn_tx_begin(TGconn* tg_conn) noexcept {
    ...
    if (!tg_conn) {
        return set_error(tg_conn->error, 
                "Invalid arguments in tg_conn_tx_begin.", TG_STATUS_INVALID_ARG);
    }
    if (!tg_conn->impl) {
        return set_error(tg_conn->error,
                "Not connected to Tsurugi.", TG_STATUS_INVALID_ARG);
    }
    if (tg_conn->subxact_seen) {
        return set_error(tg_conn->error,
                "Sub transaction is not supported.", TG_STATUS_ERROR);
    }
...
TG_STATUS tg_conn_tx_commit(TGconn* tg_conn) noexcept {
    ...
    if (!tg_conn_tx_active(tg_conn)) {
        return set_error(tg_conn->error, 
                "Remote transaction is not active.", TG_STATUS_INVALID_ARG);
    }
    try {
...
TGstmt* tg_stmt_prepare(TGconn* tg_conn, const char* sql) noexcept {
    ...
    if (!tg_conn_tx_active(tg_conn)) {
        set_error(tg_conn->error, 
            "Remote transaction is not active.", TG_STATUS_NOT_ACTIVE);
        return nullptr;
    }
...
TGresult* tg_stmt_execute_query (TGstmt* tg_stmt) noexcept {
    ...
    try {
        ...
        if (error != ERROR_CODE::OK) {
            auto msg = tg_make_error_message(tg_conn, 
                    "Failed to execute the query on Tsurugi.", 
                    error, tg_stmt->sql);
...
TG_STATUS tg_get_list_tables(TGconn* tg_conn, TableListPtr& tables) noexcept {
	...
	try {
        ...
        if (error != ERROR_CODE::OK) {
            auto msg = tg_make_error_message(tg_conn, 
                    "Failed to execute get_list_table().", error);
 ...
 *  @note Use tg_global_error_message() to retrieve the error message.
 */
TG_STATUS tg_get_table_metadata(TGconn* tg_conn, 
        const std::string& table_name, TableMetadataPtr& table_metadata) noexcept {
    elog(DEBUG3, "tsurugi_fdw: %s", __func__);
    try {
...
TG_STATUS tg_exec_import_foreign_schema(TGconn* tg_conn, 
        ImportForeignSchemaStmt* stmt, Oid serverOid, List** commands) noexcept {
    elog(DEBUG1, "tsurugi_fdw: %s", __func__);
...
TG_STATUS tg_exec_verify_tables(
	...
	std::unordered_map<
            std::string, std::vector<std::tuple<std::string, std::string, int, int>>> 
