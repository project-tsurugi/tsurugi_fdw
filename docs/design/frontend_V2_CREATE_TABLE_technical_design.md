...
### 処理フロー
* エラー処理のフローを次に示す。
1. PostgreSQLはエラーを検出する。エラーを検出した場合、エラーメッセージを出力し、2.以降は実行されない。
2. PostgreSQLは、frontendにテーブル定義要求を送信する。
3. frontendは、Tsurugiでサポートしない構文・型をチェックする。  
  サポートしない構文エラー・型エラーが起こった場合、エラーメッセージを出力し、4.以降は実行されない。
4. frontendは、metadata-managerに実テーブル定義要求を送信する。
5. frontendは、metadata-managerから戻り値でエラーコードを受け取る。  
  失敗時のエラーコードを受け取った場合、frontendは、エラーメッセージを出力し、6.以降は実行されない。
6. frontendは、ogawayamaにテーブル定義要求を送信する。
7. frontendは、ogawayamaから戻り値でエラーコードを受け取る。  
  失敗時のエラーコードを受け取った場合、frontendは、エラーメッセージを出力し、metadata-managerに4.で新規追加したテーブルのメタデータの削除要求を送信する。
8. frontendは、metadata-managerから戻り値でエラーコードを受け取る。  
  失敗時のエラーコードを受け取った場合、frontendは、エラーメッセージを出力する。
9. ユーザーの入力要求を受け取るまでの待機状態となる。
