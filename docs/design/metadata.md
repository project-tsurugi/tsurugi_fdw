...
一方、統合メタデータ管理基盤は、テーブル、インデックス、ビューなどを別々のテーブルで管理する設計思想である。また、統合メタデータ管理基盤におけるDDLメタデータの管理対象はDDL文の構成要素に絞られるため、各データベースオブジェクトの状態に関するメータデータは省くものとする（それらのメタデータはOLTP側で管理されることを想定する）。
