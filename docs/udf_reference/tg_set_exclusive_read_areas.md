...
- tg_set_exclusive_read_areas - Longトランザクションにおいて先行トランザクションの読み込み制限テーブルを設定する
...
TsurugiのLongトランザクションでは、先行しているトランザクションにおいて特定のテーブルを読み込むことを制限することができます。`tg_set_exclusive_read_areas`は先行トランザクションで特定のテーブルに対する読み込みを制限する設定をトランザクション特性に追加します。

トランザクション種別が`'long'`以外のトランザクションに対して読み込み制限テーブルを設定してもその値は無視されます。

### パラメータ

#### *table name*

先行トランザクションで読み込みを制限するテーブル名を記述します。複数のテーブル名を記述することができます。
