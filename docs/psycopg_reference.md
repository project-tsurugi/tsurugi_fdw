...
表の動作確認列は Tsurugiへのアクセス確認において使用したかどうかを表しています。  
問題なく使用できたメソッドには「〇」使用していないメソッドには「－」Tsurugiの制約により使用できないメソッドには「×」を記入しています。  
...
| `scroll(...)`            | `mode` に応じて結果セット内のカーソルを新しい位置にスクロールする | － |
...
| `closed` (属性)          | カーソルが閉じているかどうか（True）または閉じられていないか（False）を指定する読み取り専用ブール属性 | － |
| `connection` (属性)      | カーソルが作成された接続オブジェクトへの参照を返す読み取り専用属性 | － |
...
| `executemany(...)`   | 複数のパラメータセットに対して同じSQLステートメントを複数回実行する | － |
...
| `description` (属性) | 最後のクエリ結果の列のメタデータを記述するタプルのシーケンス | － |
